...
import { useTematikData } from "./useFetchSasaranPemdaData";
...
export function ModalSasaranPemda({
  isOpen,
  onClose,
  id,
  tahun,
  ...
  const tematikData = useTematikData(subtema_id);

  // Insert indikator when tematikData.indikator is available
  useEffect(() => {
    if (tematikData?.indikator && tematikData.indikator.length > 0) {
      const formattedIndikators = tematikData.indikator.map((item) => ({
        id: uuidv4(), // Generate a unique ID for React keys
        indikator: item.nama_indikator || "",
        rumus_perhitungan: "",
        sumber_data: "",
        target: tahun_list.map((tahun) => {
      ...
      replace(formattedIndikators); // Replace the entire indikator list
    }
  }, [tematikData, tahun_list, replace]); // Runs when tematikData changes
  ...
  const fetchOptionTujuanPemda = async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/tujuan_pemda/findall/${tahun}/${jenis_periode}`,
        {
          headers: {
            Authorization: `${token}`,
            "Content-Type": "application/json",
    ...
    if (TujuanPemda?.value == null || TujuanPemda?.value == undefined) {
      ...
      try {
        ...
        if (result.code === 200 || result.code === 201) {
          AlertNotification(
            "Berhasil",
            `Berhasil ${metode === "baru" ? "Menambahkan" : "Mengubah"} Sasaran Pemda`,
            "success",
            1000,
          );
          onClose();
          ...
          AlertNotification(
            "Gagal",
            "terdapat kesalahan pada backend / database server dengan response !ok",
            "error",
            50000,
            true,
          );
  ...
  if (!isOpen) {
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center z-50">
        ...
        ></div>
        <div
          className={`bg-white rounded-lg p-8 z-10 w-5/6 max-h-[80%] overflow-auto`}
        >
          <div className="w-max-[500px] py-2 border-b">
            <h1 className="text-xl uppercase text-center">
              {metode === "baru" ? "Tambah" : "Edit"} Sasaran Pemda
            </h1>
          </div>
          {Loading ? (
            <div className="mt-3">
            ...
            <div className="flex flex-wrap items-center justify-center">
              <h1 className="py-5">
                Sasaran Pemda dengan ID : {id} tidak ditemukan / telah terhapus.
                disarankan untuk reload halaman
              </h1>
            ...
            <div className="flex flex-wrap items-center justify-center">
              <h1 className="py-5">
                Tujuan Pemda telah terhapus pada sasaran pemda ini, tambahkan
                ulang sasaran baru dengan tujuan pemda yang berbeda
              </h1>
              <ButtonRed className="w-full my-2" onClick={handleClose}>
                Tutup
              </ButtonRed>
